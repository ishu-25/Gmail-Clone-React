...
export default function Message() {
    ...
    const send = async() =>{
        ...
        try {
            await addDoc(messageRef,{email:message})
        } catch (err) {
            console.error(err)
        }
    }
    ...
    return (
        <div>
            <div onClick={handleOpen} style={{ height: '4vw', marginLeft: '1vw', width: '12vw', display: 'flex', alignItems: 'center', borderRadius: '20px', backgroundColor: '#BEE0FF' }}>
                <img src={pen} style={{ width: '1.2vw', marginLeft: '2vw' }} />
                <h4 style={{ marginLeft: '1.6vw', fontWeight: '400', fontSize: '1.1vw' }}>Compose</h4>
            </div>
            <Modal
                open={open}
                onClose={handleClose}
            ...
            >
                <Box sx={style}>
                    <Typography style={{ backgroundColor: "#EDF9FF", position: 'absolute', top: "0", left: "0", width: "38vw", padding: "0.5vw", fontSize: "1vw" }}>
                        New Message
                    </Typography>
                    ...
                    <Button onClick={inbox} variant='contained' sx={{ borderRadius: "6vw", fontSize:'1vw' ,width: "4vw",height:"2.6vw"}}>Send</Button>
                </Box>
            </Modal>
        </div>
    );
